...
<body>
	<h1>Ajax</h1>
	<p>
		<i>Asynchronous JavaScript AND XML</i> <br> 서버로부터 데이터를 가져와 전체
		페이지를 바꾸지 않고 일부만 로드하여 변경하도록 하는 기법 <br> 기존에는 a태그를 사용하거나, form 태그의
		submit 요청 방식은 "동기식" 요청이다. <br> [동기식 요청] => 응답 페이지가 전달되어야 확인가능
	</p>

	<h3>* 동기식 VS 비동기식</h3>
	- 동기식
	<ul>
		<li>요청 처리 후 해당 응답 페이지가 전달된 후에 다음 작업이 가능하다.</li>
		<li>서버에 요청한 결과에 따라 응답이 지연되면 무작정 기다려야 함 (지연 시 화면이 하얗게 표시됨)</li>
	...
	<ol>
		...
		<li>요청 시 다른 페이지로 변경되지 않음 => 페이지의 깜박거림 없음!!</li>
		<li>요청을 하고 응답(데이터)가 돌아올 때까지 기다리지 않고, 다른 작업을 수행할 수 있음</li>
		<li>ex) 검색어 자동완성 기능, 아이디 중복체크, 찜 기능, 공감 및 추천, 댓글</li>
	</ol>
	...
	<ol>
		...
		<li>요청 처리 후 응답된 데이터를 가지고 새로운 요소(태그)를 만들어서 화면에 표시해야 함! <br>
	...
	<ul>
		...
		<li>jQUery 방식 (코드가 간결하고 사용하기 휘움)</li>
	</ul>
	...
	<pre>
		* jQuery 방식으로 AJAX 통신
		...
		- 주요 속성
			+ url	: 요청 주소
			+ type/method : 요청 전송방식(get, post, . . .())
			+ date : 요청 시 전달할 값
			
			+ success : 통신 성공 시 실행할 함수를 정의
			+ error : 통신 실패 시 
			+ comlete : 통신 성공/실패 상관없이 무조건 실행 할 함수를 정의
	</pre>

	<hr>

	<h3>jQuery 방식을 이용한 AJAX 테스트</h3>
	<h4>1. get방식으로 서버에 요청 및 응답 받기</h4>
	...
	<script>
		// [전송] 클릭되었을 때 /jqAjax1.do로 입력된 값을 전송 (요청)
		$("#btn1").click(function() {
			// 기존 방식 (동기식)
			...
			$.ajax({
				...
				success: function(result) {
					// 통신 성공 시 실행할 내용
					// => 응답 데이터(결과)가 result라는 변수에 담겨져 있을 것임!
	...
	<br> 응답 결과 :
	<div id="result2">응답 없음</div>

	<script>
		$("#btn2").click(function() {
		// [전송] 클릭되었을 때 입력된 값들(이름, 나이)을 jqAjax2.do 로 전송 (post)
			...
			$.ajax({
				...
				success: function(result) {
					...
					*/
					...
					// => 결과가 배열형태 이므로 반복문을 사용하여 요소 추가 가능!
					*/
					
					// * 일반 객체(JSONObject)의 응답된 데이터 처리
